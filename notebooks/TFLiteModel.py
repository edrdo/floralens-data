...
def top1(data):
    r = pd.DataFrame()
    r['top1'] = (data['species'] == data['rank1']).astype(float) 
    return r['top1'].mean()

def top5(data):
    r = pd.DataFrame()
    r['top5'] = (data['species'] == data['rank1']).astype(float) + \
                    (data['species'] == data['rank2']).astype(float)  + \
                        (data['species'] == data['rank3']).astype(float)  + \
                        (data['species'] == data['rank4']).astype(float)  + \
                            (data['species'] == data['rank5']).astype(float) 
    return r['top5'].mean()

def mrr(data):
    r = pd.DataFrame()
    r['mrr'] = (data['species'] == data['rank1']).astype(float) + \
...
class Model:
    ...
    def predict(self, files):
        results = []
        for file in files:
            img = load_image(file).resize((self.width, self.height))
            ...
            if results == []:
                ...
                results = results + output_data
        return results / len(files)
    def classify(self, files, max_results, min_confidence):
        results = self.predict(files)
